...
spinsRouter.post('/:spinId/video', upload.single('video'), async (request, response) => {
    ...
    const existingVideo = await database.get('SELECT video_path FROM videos WHERE spin_id = ?', [spinId]);
    const relativePath = path.posix.join('storage', 'sessions', request.file.filename);
    await database.run(`INSERT INTO videos (spin_id, video_path) VALUES (?, ?)
     ON CONFLICT(spin_id) DO UPDATE SET video_path = excluded.video_path`, [spinId, relativePath]);
...
spinsRouter.get('/:spinId', async (request, response) => {
    const spinId = parseIdParam(request.params.spinId, 'spinId');
    const database = await getDatabase();
    const spin = await database.get(`SELECT id, session_id, spin_number, win_amount, cascades, bonus_triggered, duration, start_frame, end_frame
     FROM spins WHERE id = ?`, [spinId]);
    if (!spin) {
        return response.status(404).json({ error: 'Spin not found' });
    }
    const events = await database.all('SELECT id, spin_id, event_type, timestamp FROM events WHERE spin_id = ? ORDER BY timestamp ASC', [spinId]);
    const video = await database.get('SELECT video_path FROM videos WHERE spin_id = ?', [spinId]);
