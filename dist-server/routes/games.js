...
gamesRouter.post('/', async (request, response) => {
    ...
    const game = await database.get('SELECT id, name, provider, game_type, assigned_rtp FROM games WHERE id = ?', [result.lastID]);
    return response.status(201).json(game);
});
gamesRouter.get('/', async (_request, response) => {
    const database = await getDatabase();
    const games = await database.all(`SELECT games.id, games.name, games.provider, games.game_type, games.assigned_rtp, COUNT(sessions.id) AS total_sessions
     FROM games
...
gamesRouter.put('/:gameId', async (request, response) => {
    ...
    const updatedGame = await database.get(`SELECT games.id, games.name, games.provider, games.game_type, games.assigned_rtp, COUNT(sessions.id) AS total_sessions
     FROM games
     LEFT JOIN sessions ON sessions.game_id = games.id
     WHERE games.id = ?
...
gamesRouter.get('/:gameId/base-breakdown', async (request, response) => {
    ...
    const row = await database.get('SELECT game_id, breakdown_json, updated_at, updated_by_email FROM game_base_breakdowns WHERE game_id = ?', [gameId]);
    if (!row) {
...
gamesRouter.put('/:gameId/base-breakdown', async (request, response) => {
    ...
    await database.run(`INSERT INTO game_base_breakdowns (game_id, breakdown_json, updated_at, updated_by_email)
     VALUES (?, ?, CURRENT_TIMESTAMP, ?)
     ON CONFLICT(game_id) DO UPDATE SET breakdown_json = excluded.breakdown_json, updated_at = CURRENT_TIMESTAMP, updated_by_email = excluded.updated_by_email`, [gameId, json, userEmail]);
    const row = await database.get('SELECT updated_at, updated_by_email FROM game_base_breakdowns WHERE game_id = ?', [gameId]);
...
gamesRouter.get('/:gameId/bonus-breakdown', async (request, response) => {
    ...
    const row = await database.get('SELECT game_id, breakdown_json, updated_at, updated_by_email FROM game_bonus_breakdowns WHERE game_id = ?', [gameId]);
    if (!row) {
        return response.json({ gameId, breakdown: null, updatedAt: null, updatedByEmail: null });
    }
    return response.json({
...
gamesRouter.put('/:gameId/bonus-breakdown', async (request, response) => {
    ...
    const json = JSON.stringify(breakdown);
    await database.run(`INSERT INTO game_bonus_breakdowns (game_id, breakdown_json, updated_at, updated_by_email)
     VALUES (?, ?, CURRENT_TIMESTAMP, ?)
     ON CONFLICT(game_id) DO UPDATE SET breakdown_json = excluded.breakdown_json, updated_at = CURRENT_TIMESTAMP, updated_by_email = excluded.updated_by_email`, [gameId, json, userEmail]);
    const row = await database.get('SELECT updated_at, updated_by_email FROM game_bonus_breakdowns WHERE game_id = ?', [gameId]);
    return response.json({ gameId, breakdown, updatedAt: row?.updated_at ?? null, updatedByEmail: row?.updated_by_email ?? null });
});
