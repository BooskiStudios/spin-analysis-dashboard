...
import { parseEvents, parseIdParam, parseOptionalNumber, parseRequiredBoolean, parseRequiredNumber, } from '../lib/validation.js';
...
sessionsRouter.post('/', async (request, response) => {
    ...
    const result = await database.run('INSERT INTO sessions (game_id, total_spins, rtp, created_at) VALUES (?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP))', [gameId, totalSpins, rtp, createdAt ?? null]);
    const session = await database.get('SELECT id, game_id, total_spins, rtp, created_at FROM sessions WHERE id = ?', [result.lastID]);
    return response.status(201).json(session);
});
sessionsRouter.get('/:gameId', async (request, response) => {
    const gameId = parseIdParam(request.params.gameId, 'gameId');
    const database = await getDatabase();
    const sessions = await database.all(`SELECT sessions.id, sessions.game_id, sessions.total_spins, sessions.rtp, sessions.created_at,
     ...
     GROUP BY sessions.id, sessions.game_id, sessions.total_spins, sessions.rtp, sessions.created_at,
              sessions.source_video_path, sessions.processing_status, sessions.processing_error
     ORDER BY datetime(sessions.created_at) DESC`, [gameId]);
...
sessionsRouter.post('/:sessionId/spins', async (request, response) => {
    ...
    try {
        const spinResult = await database.run(`INSERT INTO spins (session_id, spin_number, win_amount, cascades, bonus_triggered, duration)
       VALUES (?, ?, ?, ?, ?, ?)`, [sessionId, spinNumber, winAmount, cascades, bonusTriggered ? 1 : 0, duration]);
        for (const event of events) {
            if (!event.type || event.time === undefined) {
                continue;
            }
            await database.run('INSERT INTO events (spin_id, event_type, timestamp) VALUES (?, ?, ?)', [spinResult.lastID, event.type, event.time]);
        }
        await database.run('UPDATE sessions SET total_spins = (SELECT COUNT(*) FROM spins WHERE session_id = ?) WHERE id = ?', [sessionId, sessionId]);
        await database.exec('COMMIT');
        const spin = await database.get(`SELECT id, session_id, spin_number, win_amount, cascades, bonus_triggered, duration
       FROM spins WHERE id = ?`, [spinResult.lastID]);
...
sessionsRouter.get('/:sessionId/spins', async (request, response) => {
    const sessionId = parseIdParam(request.params.sessionId, 'sessionId');
    const database = await getDatabase();
    const spins = await database.all(`SELECT spins.id, spins.session_id, spins.spin_number, spins.win_amount, spins.cascades,
     ...
     ORDER BY spins.spin_number ASC`, [sessionId]);
    return response.json(spins.map((spin) => ({
        ...spin,
