...
import { processUploadedSessionVideo } from '../lib/sessionVideoProcessor.js';
...
function parseUploadedSessionMetadata(rawMetadata) {
...
async function getOrCreateUploadsGame(gameId) {
    ...
    const uploadedGame = await database.get('SELECT id FROM games WHERE name = ? AND provider = ?', ['Uploaded Session Videos', 'User Upload']);
    if (uploadedGame) {
...
uploadsRouter.post('/upload-session-video', upload.single('video'), async (request, response) => {
    ...
    const activeUpload = await database.get(`SELECT id
     FROM sessions
    ...
    const result = await database.run(`INSERT INTO sessions (game_id, total_spins, rtp, created_at, source_video_path, processing_status)
     VALUES (?, 0, 0, CURRENT_TIMESTAMP, ?, ?)`, [gameId, relativeSourceVideoPath, 'queued']);
