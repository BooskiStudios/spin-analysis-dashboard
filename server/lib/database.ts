...
async function ensureColumn(database: Database, tableName: string, columnDefinition: string) {
  ...
  const tableInfo = await database.all<Array<{ name: string }>>(`PRAGMA table_info(${tableName})`)
