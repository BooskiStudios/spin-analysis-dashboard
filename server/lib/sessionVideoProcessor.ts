...
type SpinSegment = {
  ...
  startFrame: number
  endFrame: number
  metadata?: Partial<SpinExtractedMetadata & Pick<SegmentMotionMetadata, 'cascades'>>
}
...
type SegmentMotionMetadata = {
  cascades: number
  cascadeTimes: number[]
}

type SpinExtractedMetadata = {
  winAmount: number
...
export type UploadedSessionMetadata = {
  totalSpins?: number
  spins?: UploadedSpinMetadata[]
}
...
function parseWinAmount(text: string) {
...
function detectBonusKeyword(text: string) {
...
function calculateAnalysisFps(durationSeconds: number) {
...
function normalizeMetadataTime(value: number | undefined) {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : null
}

function buildSegmentsFromMetadata(
  metadata: UploadedSessionMetadata | undefined,
  durationSeconds: number,
  framesPerSecond: number,
): SpinSegment[] | null {
...
function buildSegmentsFromBoundaries(
  ...
  const pushSegment = (segments: SpinSegment[], startSeconds: number, endSeconds: number) => {
  ...
  for (let index = 0; index < normalizedBoundaries.length - 1; index += 1) {
    ...
    const sceneCandidates = sceneTimes.filter(
      (timestamp) => timestamp > startSeconds + minSpinDurationSeconds && timestamp < endSeconds - minSpinDurationSeconds,
    )
...
function detectSegmentMotionMetadata(frameSamples: FrameSample[], segment: SpinSegment): SegmentMotionMetadata {
  const segmentSamples = frameSamples.filter(
    (sample) => sample.timestampSeconds >= segment.startSeconds && sample.timestampSeconds <= segment.endSeconds,
  )
  ...
  const smoothedScores = smoothValues(
    segmentSamples.map((sample) => sample.motionScore),
    1,
  )
...
function findFrameNearTime(frameSamples: FrameSample[], targetTimeSeconds: number) {
...
async function recognizeText(worker: Worker, imageBuffer: Buffer, whitelist: string, pageSegMode: PSM) {
  ...
  const result = await withTimeout(worker.recognize(imageBuffer), 1500, { data: { text: '' } } as Awaited<ReturnType<Worker['recognize']>>)
...
async function extractSpinGameplayMetadata(worker: Worker, frameSamples: FrameSample[], segment: SpinSegment): Promise<SpinExtractedMetadata> {
...
async function runFfmpeg(args: string[], acceptedExitCodes = [0]) {
...
async function extractFrames(videoPath: string, framesDirectory: string, analysisFps: number) {
...
async function detectSceneTimes(videoPath: string, analysisFps: number) {
  const { stderr } = await runFfmpeg(
    ['-i', videoPath, '-vf', `fps=${analysisFps},select=gt(scene\\,0.1),showinfo`, '-fps_mode', 'vfr', '-f', 'null', getNullDevice()],
    [0],
  )
...
async function buildFrameSamples(framesDirectory: string, analysisFps: number) {
...
async function createSpinClip(sourceVideoPath: string, outputPath: string, startSeconds: number, durationSeconds: number) {
  await runFfmpeg([
    '-y',
    '-ss',
...
async function removeExistingSessionSpins(sessionId: number) {
  const database = await getDatabase()
  const spinIds = await database.all<Array<{ id: number }>>('SELECT id FROM spins WHERE session_id = ?', [sessionId])
...
export async function processUploadedSessionVideo(sessionId: number, sourceVideoPath: string, metadata?: UploadedSessionMetadata) {
  const database = await getDatabase()
  ...
  try {
    ...
    const segments =
      buildSegmentsFromMetadata(metadata, videoProbe.durationSeconds, videoProbe.framesPerSecond) ??
    ...
    for (const segment of segments) {
      ...
      const motionMetadata =
        segment.metadata?.cascades !== undefined
          ? { cascades: segment.metadata.cascades, cascadeTimes: [] }
          : detectSegmentMotionMetadata(frameSamples, segment)
      const extractedMetadata =
        segment.metadata?.winAmount !== undefined && segment.metadata?.bonusTriggered !== undefined
          ? {
      ...
      const spinResult = await database.run(
        `INSERT INTO spins (session_id, spin_number, win_amount, cascades, bonus_triggered, duration, start_frame, end_frame)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          sessionId,
          segment.spinNumber,
      ...
      if (extractedMetadata.bonusTriggered) {
        await database.run(
          ...
          [spinResult.lastID, 'bonus_detected', Math.max(0, duration - 0.2)],
        )
      }
    ...
    await database.run(
      `UPDATE sessions
       ...
       WHERE id = ?`,
      [segments.length, 'completed', sessionId],
    )
  } catch (error) {
    await database.run(
      'UPDATE sessions SET processing_status = ?, processing_error = ? WHERE id = ?',
